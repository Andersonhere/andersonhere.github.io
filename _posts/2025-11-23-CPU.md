...
> **扩展阅读**：本文关于 CFS 调度算法的内容已整理为独立笔记 [Linux进程调度详解](2025-04-16-Linux-进程调度详解.md)，包含更详细的实现细节和实践案例。

#### 7.1 CFS概述

CFS（Completely Fair Scheduler，完全公平调度器）从 Linux 2.6.23 开始成为默认调度器，取代了之前的 O(1) 调度器。其核心设计理念是：**在理想的多任务CPU上，每个进程都能精确地按比例获得CPU时间**。

CFS的主要特点：

1. **基于红黑树**：所有可运行进程按虚拟运行时间（vruntime）组织在红黑树中
2. **公平性**：vruntime最小的进程优先运行，确保每个进程公平获得CPU
3. **权重系统**：通过权重将优先级（nice值）转换为CPU时间比例
4. **无时间片概念**：动态计算每个进程应运行的时间

#### 7.2 虚拟运行时间（vruntime）

vruntime 是 CFS 的核心度量指标：

```
vruntime += 实际运行时间 × (标准权重 / 进程权重)
```

- 高优先级进程权重更大，vruntime增长更慢，能获得更多CPU时间
- CFS 总是选择红黑树中最左侧（vruntime最小）的进程运行
...
实时进程优先级（0-99）高于普通进程（100-139），保证实时任务的及时响应。

---

**相关笔记**：
- [Linux进程调度详解](2025-04-16-Linux-进程调度详解.md) - CFS实现细节、调度类、负载均衡
- [操作系统概述](2025-11-16-O-system.md) - 操作系统虚拟化概念介绍
